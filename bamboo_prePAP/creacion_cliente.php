...
<!-- body code goes here "/bamboo_prePAP/backend/clientes/crea_cliente.php"-->
<div id="header">
...
<div class="container">
  ...
  <form action="/bamboo_prePAP/backend/clientes/crea_cliente.php" class="needs-validation" method="POST" id="formulario"novalidate>
...
function valida_rut_duplicado() {
    if (document.getElementById("rut").checkValidity() == true) {
        ...
        $.ajax({
            type: "POST",
            url: "/bamboo_prePAP/backend/clientes/clientes_duplicados.php",
            data: {
                rut: rut_sin_dv
            },
            dataType: 'JSON',
            success: function(response) {
                ...
                if (response.resultado == 'duplicado') {
                    ...
                    if (r == true) {
                        $.redirect('/bamboo_prePAP/listado_clientes.php', {
                            'busqueda': rut_sin_dv
                        }, 'post');

                    } else {
                        location.href =
                            "http://gestionipn.cl/bamboo_prePAP/creacion_cliente.php";
                        $.notify({
                            // options
      ...
      var info_contact = <?php echo json_encode($contactos_array);?>;
       ...
       if( cant_contact !== 0){
        ...
        document.getElementById("rut").disabled ="true";
		 ...
		 document.getElementById("formulario").action="/bamboo_prePAP/backend/clientes/modifica_cliente.php";
