...
//if ( $_SERVER[ "REQUEST_METHOD" ] == "POST" ) {
  ...
  //if ( !empty( trim( $_POST[ "id_poliza" ] ) ) ) {
    ...
    $query =  "SELECT  a.id, estado, tipo_poliza, rut_proponente, dv_proponente, rut_asegurado, dv_asegurado, a.grupo, compania, vigencia_inicial, vigencia_final, mes_vencimiento, ano_vencimiento, poliza_renovada, ramo, numero_poliza, materia_asegurada, patente_ubicacion, cobertura, deducible, moneda_poliza, FORMAT(prima_bruta_anual, 2, 'de_DE') as prima_bruta_anual, FORMAT(prima_afecta, 2, 'de_DE') as prima_afecta, moneda_comision, FORMAT(prima_exenta, 2, 'de_DE') as prima_exenta, FORMAT(prima_neta, 2, 'de_DE') as prima_neta, FORMAT(prima_bruta_anual, 2, 'de_DE') as prima_bruta_anual, monto_asegurado, numero_propuesta, fecha_envio_propuesta, endoso, FORMAT(comision, 2, 'de_DE') as comision, FORMAT(porcentaje_comision, 2, 'de_DE') as porcentaje_comision, FORMAT(comision_bruta, 2, 'de_DE') as comision_bruta, FORMAT(comision_neta, 2, 'de_DE') as comision_neta, numero_boleta, moneda_comision_negativa, FORMAT(comision_negativa, 2, 'de_DE') as comision_negativa, boleta_negativa, depositado_fecha, vendedor, nombre_vendedor, forma_pago, moneda_valor_cuota, FORMAT(valor_cuota, 2, 'de_DE') as valor_cuota, fecha_primera_cuota, nro_cuotas, informacion_adicional, concat_ws(' ',b.nombre_cliente, b.apellido_paterno) as nombre_asegurado FROM polizas as a left join clientes as b on a.rut_asegurado=b.rut_sin_dv where a.id=" . $busqueda . " order by compania, numero_poliza;"; 
