...
require_once "/home/gestio10/public_html/backend/config.php";
    ...
    $codigo='{
      "data": [';
    $conta=0;
    $resul_tareas=mysqli_query($link, "SELECT a.id, fecha_ingreso, fecha_vencimiento, tarea, estado, prioridad, count(b.id) as relaciones, sum(CASE WHEN base ='polizas' THEN 1 ELSE 0 END) as polizas, sum(CASE WHEN base ='clientes' THEN 1 ELSE 0 END) as clientes FROM tareas as a left join tareas_relaciones as b on a.id=b.id_tarea WHERE estado <>'Completado' group by a.id, fecha_ingreso, fecha_vencimiento, tarea, estado, prioridad");
  ...
  {$conta=$conta+1;
    ...
    if (!$tareas->relaciones=="0"){
        ...
        $ramo=$estado=$vigencia_inicial=$vigencia_final=$compania=$numero_poliza=$materia_asegurada=$patente_ubicacion=$cobertura=$nom_clienteP=$rut_clienteP=$telefonoP=$correoP=$nom_clienteA=$rut_clienteA=$telefonoA=$correoA=$id_proponente=$id_asegurado=$id_poliza=array();
